...
export default function GetServicePage() {
  ...
  const handlePlanSelect = (planName) => {
    ...
    setFormData(prev => ({ ...prev, budget: planName }));
        ...
        {selectedService && (
          <div className="max-w-6xl mx-auto mb-16">
            ...
            <div className="grid md:grid-cols-4 gap-6">
              {services.find((s) => s.id === selectedService)?.process.map((step, index) => (
                <div key={index} className="text-center group">
                  <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-r from-purple-600 to-pink-600 flex items-center justify-center text-white font-bold text-xl">
                    {step.step}
                  </div>
                  <h4 className="font-semibold text-white mb-2">{step.title}</h4>
                  <p className="text-gray-400 text-sm">{step.desc}</p>
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {pricingPlans.map((plan, index) => (
                <div
                  key={index}
                  className={`relative bg-gradient-to-br from-gray-800/30 to-black border ${
                    selectedPlan === plan.name ? "border-green-400" : plan.popular ? "border-purple-500" : "border-gray-700"
                  } rounded-2xl p-6 hover:scale-105 transition-all duration-300`}
                >
                  {plan.popular && (
                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                  ...
                  <div className="text-center mb-6">
                    ...
                    <div className="flex items-baseline justify-center space-x-1">
                      <span className="text-3xl font-bold text-white">{plan.price}</span>
                      {plan.period !== "Custom" && <span className="text-gray-400">/{plan.period}</span>}
                    </div>
                  </div>
                  ...
                  <button
                    className={`w-full py-3 px-6 rounded-xl font-semibold bg-gradient-to-r ${plan.gradient} text-white hover:scale-105`}
                    onClick={() => handlePlanSelect(plan.name)}
                  >
                    {selectedPlan === plan.name ? "Selected ✓" : plan.price === "Custom" ? "Get Quote" : "Select Plan"}
                  </button>
                </div>
              ))}
            </div>
        ...
        <div ref={formRef} className="max-w-4xl mx-auto" id='service-form'>
          <div className="bg-gradient-to-br from-gray-900 to-black border border-gray-800 rounded-3xl p-8 md:p-12">
            ...
            <form onSubmit={handleSubmit} className="space-y-6" >
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-gray-300 text-sm font-medium mb-2">Service Interested In *</label>
                  <select
                    name="service"
                    required
                    value={formData.service}
                ...
                <div>
                  <label className="block text-gray-300 text-sm font-medium mb-2">Expected Budget</label>
              ...
              <div>
                <label className="block text-gray-300 text-sm font-medium mb-2">Project Timeline</label>
                <select
                  name="timeline"
                  value={formData.timeline}
                  onChange={handleInputChange}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 bg-black/50 border border-gray-700 rounded-xl text-white resize-none"
                ></textarea>
              </div>

              <button
                type="submit"
                className="w-full py-4 px-6 rounded-xl font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:scale-105 transition-transform"
              >
                Submit Your Inquiry
              </button>
